...
import { MessageCircle, X, Send, User, Calendar, XCircle, Info, Phone } from 'lucide-react';
  ...
  return (
    <>
      ...
      <Button
        ...
        size="chatIcon"
        className={cn(
          'fixed bottom-6 right-6 z-50 transition-all duration-300 shadow-cyber animate-glow',
      ...
      <div
        className={cn(
          'fixed bottom-6 right-6 z-50 w-[400px] h-[600px] max-h-[80vh] bg-card rounded-2xl shadow-chat border border-border/50 flex flex-col overflow-hidden transition-all duration-300 origin-bottom-right',
          isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'
        )}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/50 bg-card/80 backdrop-blur-xl">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl overflow-hidden ring-1 ring-primary/30 shadow-glow">
              <img src={agentLogo} alt="Agent14" className="w-full h-full object-cover" />
            </div>
            <div>
              <h3 className="font-display font-semibold text-foreground">Agent14</h3>
          ...
          <Button variant="ghost" size="icon" onClick={onToggle} className="rounded-full hover:bg-secondary">
            <X className="w-5 h-5" />
          </Button>
        </div>
            ...
            >
              <div
                className={cn(
                  ...
                  message.role === 'user' ? 'bg-primary' : 'ring-1 ring-primary/20'
                )}
              >
                {message.role === 'user' ? (
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {quickActions.map((action) => (
              <button
                ...
                disabled={isTyping}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-card border border-border/50 text-xs font-medium text-foreground hover:border-primary/30 hover:bg-secondary transition-all whitespace-nowrap disabled:opacity-50"
              >
        ...
        <form onSubmit={handleSubmit} className="p-4 border-t border-border/50 bg-card/80">
          <div className="flex gap-2">
            <input
              ...
              disabled={isTyping}
              className="flex-1 px-4 py-3 rounded-xl bg-secondary/50 border border-border/50 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all"
            />
            <Button type="submit" disabled={!input.trim() || isTyping} className="rounded-xl shadow-glow">
